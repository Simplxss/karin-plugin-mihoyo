...
class MihoyoPassport {
  async loginByPassword (account, password) {
    let res = await requsest(`${MIHOYO_PASSPORT_DOMAIN}/account/ma-cn-passport/web/loginByPassword`, { account: encrypt(account), password: encrypt(password) }, header['mobile'], {}, 2)
    return res.data
  }
